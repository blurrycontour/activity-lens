...
const faqs = [
  ...
  { q: 'How do training goals and streaks work?', a: 'Add as many goals as you like under Settings → Training Goals — say two runs of at least 5 km a week, plus two hikes a month. Each is tracked separately on the dashboard with its own streak of periods that met the target. The period in progress extends a streak once you hit it, but never breaks one, so a quiet Monday costs you nothing. Distance minimums are matched against the figure shown on the workout, so a run listed as 5.0 km counts toward a 5 km goal even though its recorded distance is 4,983 m.' },
