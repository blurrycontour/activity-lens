...
const faqs = [
  ...
  { q: 'How do training goals and streaks work?', a: 'Add as many goals as you like under Settings → Training Goals. A goal can count activities ("two runs of at least 5 km a week"), total distance ("40 km of hiking a month") or total time ("30 hours of running a month"), over a window of one or more weeks or months. Reorder them with the arrows — the dashboard shows them in that order. Each is tracked separately with its own streak of windows that met the target. The window in progress extends a streak once you hit it, but never breaks one, so a quiet Monday costs you nothing. Windows longer than a single week or month run back to back from a fixed anchor rather than counting back from today, so they never overlap. Distance minimums are matched against the figure shown on the workout, so a run listed as 5.0 km counts toward a 5 km goal even though its recorded distance is 4,983 m.' },
