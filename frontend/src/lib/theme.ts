...
export const ACCENTS: { name: string; value: string; dim: string; glow: string }[] = [
  ...
  /* Lifted off #f43f5e, which sat 14.7 ΔE from --danger, then again off
     #fb7185, which sat 16.7 ΔE from --strength — both close enough that Rose
     and a status or sport colour read as the same red. This is the nearest
     shade in the same rosy family that clears every one of them. */
  { name: 'Bold Rose',        value: '#f9386f', dim: 'rgba(249,56,111,0.15)', glow: 'rgba(249,56,111,0.3)' },
]
